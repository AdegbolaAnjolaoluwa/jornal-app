/**
 * Vanilla-JS port of the React Bits "LightRays" component (ogl-based WebGL
 * light-ray effect). No React here, this app has no build step, so the
 * original hooks/refs/JSX are replaced with a single init function that
 * returns a destroy() callback - same pattern as side-rays.js, which already
 * ports a different React Bits ray effect for the landing page hero.
 */
...
/**
 * Mount the light-rays effect into `container` and start animating once it's
 * on screen. Returns a destroy() function that tears down the WebGL context,
 * listeners, and observer. `container` must be position:relative (or similar)
 * since the canvas fills it via 100%/100% sizing.
 */
