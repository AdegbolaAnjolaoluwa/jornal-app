...
export const users = {
  ...
  // Narrow, single-purpose accessor for the one legitimate case where a
  // route needs to re-verify the current password of an already-
  // authenticated user (e.g. step-up confirmation before account deletion)
  // without widening findById's deliberately password_hash-free default
  // select used everywhere else.
  async getPasswordHash(id) {
    return queryOne("SELECT password_hash FROM users WHERE id = $1", [id]);
  },
  ...
  async create(email, passwordHash, { name, timezone } = {}) {
    const result = await queryOne(
      `INSERT INTO users (id, email, password_hash, name, timezone, created_at, updated_at)
       ...
       RETURNING id, email, name, timezone, onboarding_completed_at, token_version, created_at`,
  ...
  /**
   * A genuine no-op DB round-trip, same query shape (and therefore same
   * approximate cost) as setResetToken, run against a WHERE clause that
   * can never match a real row. Used only so the password-reset-request
   * handler's "no such account" path does comparable DB work to the "user
   * exists" path, closing a timing side-channel that would otherwise reveal
   * whether an email is registered (SEC-002) - it writes nothing real and
   * creates no token for anyone.
   */
  async dummyResetTokenWork() {
    return queryOne(
      "UPDATE users SET updated_at = updated_at WHERE id = $1 RETURNING id",
  ...
  // Only these columns are ever legitimately settable through this generic
  // updater (currently just name/nickname, from handleProfile in
  // api/auth.js). Enforced here, not just by the caller, so this function
  // stays safe by construction even if a future caller ever passes through
  // a client-supplied object more directly - an unlisted key (e.g.
  // password_hash, token_version, email, or an arbitrary SQL fragment used
  // as a key) is silently ignored rather than becoming a raw SQL column
  // name.
  async update(id, updates) {
    const ALLOWED_FIELDS = new Set(["name", "nickname"]);
  ...
  /**
   * Permanently delete a user and everything tied to them (entries, action
   * points, entry messages, facts, tags, recaps, ask threads/messages) - all
   * cascade at the DB level from this single DELETE, same convention as
   * entries.delete(). Only ever call this with an id from requireAuth(req),
...
export const entries = {
  ...
  async findByUserId(userId, { limit = 50, offset = 0, q = null, tagId = null, matchMode = "and" } = {}) {
    ...
    if (q) {
      ...
      values.push(q);
      // Blends text-match quality with recency: a match's rank is halved
      // for every ~90 days old it is (exp base 0.5, exponent = age/90).
      // Pure ts_rank alone lets an old, strong keyword match permanently
      // outrank a weaker-but-recent one, which is backwards for a journal -
      // "how have I been feeling lately" should favor recent entries over
      // an equally-worded entry from 8 months ago. 90 days (not 7 or 30) so
      // a same-quality match doesn't fall off within a single recap period;
      // it's a soft decay, not a hard recency filter.
      rankSelect = `, ts_rank(e.search_vector, ${tsqueryExpr}) * pow(0.5, extract(epoch FROM now() - e.created_at) / 86400 / 90) AS rank`;
...
export const actionPoints = {
  ...
  /**
   * Incomplete action points due within the next `days` days, or already
   * overdue - a user-scoped, on-demand counterpart to findDueReminders()
   * (which is deliberately cross-user/cron-only and can't be called
   * per-request). Overdue-first, then soonest-due, mirroring the same
   * ordering app.html's renderTodoList already applies client-side, so a
   * caller (e.g. the home chat) gets a small, ready-to-mention list without
   * needing to re-sort it.
   */
    ...
    return query(
      `SELECT ${ACTION_POINT_COLUMNS}
       ...
       WHERE e.user_id = $1
         ...
         AND ap.due_date <= (CURRENT_DATE + ($2 || ' days')::interval)
...
export const userFacts = {
  ...
  /**
   * Full-text search a user's facts, ranked by ts_rank. OR-mode only (same
   * regexp-rewrite trick as entries.findByUserId): a natural-language
   * question's words are ORed, not ANDed, so one absent term (the question
   * says "work", a fact says "worked" and stems match, but a genuinely
   * unrelated word in the question shouldn't sink an otherwise-good match).
   * Used by lib/retrieval.js's scoreFacts instead of a hand-rolled JS
   * word-overlap count, so fact matching gets Postgres's real stemmer.
   */
  async searchByUserId(userId, q, limit = 5) {
    return query(
      `SELECT id, user_id, text, entry_id, created_at,
              ts_rank(search_vector, to_tsquery('english', regexp_replace(plainto_tsquery('english', $2)::text, ' & ', ' | ', 'g'))) AS rank
       FROM user_facts
       WHERE user_id = $1
         AND search_vector @@ to_tsquery('english', regexp_replace(plainto_tsquery('english', $2)::text, ' & ', ' | ', 'g'))
...
export const chatThreads = {
  /**
   * Every user has at most one open chat thread (unlike ask_threads, which
   * creates a new thread per question) - the home-page chat is one ongoing
   * conversation, not a series of one-off Q&A rounds. This finds it, or
   * creates it on first use.
   */
  async findOrCreateForUser(userId) {
    const existing = await queryOne(
      `SELECT id, user_id, created_at, updated_at FROM chat_threads WHERE user_id = $1`,
      [userId]
    );
  ...
  async findById(id, userId) {
    return queryOne(
      `SELECT id, user_id, created_at, updated_at FROM chat_threads WHERE id = $1 AND user_id = $2`,
      [id, userId]
    );
  },

  async touch(id) {
    return queryOne(`UPDATE chat_threads SET updated_at = now() WHERE id = $1 RETURNING id`, [id]);
  },
};

export const chatMessages = {
  /**
   * Get all messages in a chat thread (ownership check joined through
   * chat_threads.user_id, same pattern as askMessages.findByThreadId).
   */
  async findByThreadId(threadId, userId) {
    return query(
      `SELECT m.id, m.thread_id, m.role, m.content, m.entry_id, m.created_at
  ...
  async create(threadId, userId, role, content, entryId = null) {
    const thread = await chatThreads.findById(threadId, userId);
    if (!thread) throw new Error("Thread not found");

    return queryOne(
      `INSERT INTO chat_messages (id, thread_id, role, content, entry_id, created_at)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, now())
       RETURNING id, thread_id, role, content, entry_id, created_at`,
      [threadId, role, content, entryId]
    );
  },
};
...
export async function initializeSchema() {
  try {
    ...
    // Same STORED-generated-column pattern as entries.search_vector above,
    // so fact matching (lib/retrieval.js's scoreFacts) can use Postgres's
    // real stemmer/stopword handling instead of a hand-rolled JS word-match
    // (which missed stems like "worked" not matching a question's "work").
    await sql`
      ALTER TABLE user_facts ADD COLUMN IF NOT EXISTS search_vector tsvector
      GENERATED ALWAYS AS (to_tsvector('english', coalesce(text, ''))) STORED;
    `;
    await sql`CREATE INDEX IF NOT EXISTS idx_user_facts_search_vector ON user_facts USING GIN(search_vector);`;
    ...
    // One open thread per user (find-or-create, not one-per-question like
    // ask_threads) for the home-page chat. entry_id on chat_messages is
    // nullable and ON DELETE SET NULL (not CASCADE) - if a chat message
    // gets promoted into a real journal entry and that entry is later
    // deleted, the chat message and its place in the conversation should
    // survive, just no longer pointing at a deleted entry.
    await sql`
    ...
    `;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_chat_threads_user_id ON chat_threads(user_id);`;
    await sql`CREATE INDEX IF NOT EXISTS idx_chat_messages_thread_id ON chat_messages(thread_id);`;
