...
export const users = {
  async findById(id) {
    return queryOne(
      "SELECT id, email, name, nickname, profile_picture_mime_type, created_at, updated_at FROM users WHERE id = $1",
      [id]
    );
  },
  ...
  async findByEmail(email) {
    return queryOne(
      "SELECT id, email, password_hash, name, nickname, created_at, updated_at FROM users WHERE email = $1",
      [email]
    );
  },
