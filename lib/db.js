...
export const entries = {
  ...
  async findArchivedByUserId(userId, limit = 50, offset = 0) {
    return query(
      `SELECT id, user_id, input_type, input_text, title, reflection, clarifying_question, has_audio, is_archived, archived_at, is_deleted, created_at, updated_at
       FROM entries
  ...
  async findTrashedByUserId(userId, limit = 50, offset = 0) {
    return query(
      `SELECT id, user_id, input_type, input_text, title, reflection, clarifying_question, has_audio, is_archived, is_deleted, deleted_at, created_at, updated_at
       FROM entries
  ...
  async archive(id, userId) {
    return queryOne(
      `UPDATE entries
       ...
       WHERE id = $1 AND user_id = $2
       RETURNING id, user_id, input_type, input_text, title, reflection, clarifying_question, has_audio, is_archived, archived_at, created_at, updated_at`,
      [id, userId]
    );
  },
  ...
  async softDelete(id, userId) {
    return queryOne(
      `UPDATE entries
       ...
       WHERE id = $1 AND user_id = $2
       RETURNING id, user_id, input_type, input_text, title, reflection, clarifying_question, has_audio, is_archived, is_deleted, deleted_at, created_at, updated_at`,
      [id, userId]
    );
  },
...
const ACTION_POINT_COLUMNS =
  "ap.id, ap.entry_id, ap.text, ap.completed, ap.remind_at, ap.reminder_sent_at, ap.due_date::text AS due_date, ap.created_at, ap.updated_at";
  ...
  async create(entryId, userId, text, dueDate = null) {
    ...
    return queryOne(
      `INSERT INTO action_points (id, entry_id, text, completed, due_date, created_at, updated_at)
       ...
       RETURNING id, entry_id, text, completed, remind_at, reminder_sent_at, due_date::text AS due_date, created_at, updated_at`,
  ...
  async update(apId, userId, { completed, remindAt, dueDate } = {}) {
    ...
    const result = await queryOne(
      `UPDATE action_points ap
       ...
       RETURNING ap.id, ap.entry_id, ap.text, ap.completed, ap.remind_at, ap.reminder_sent_at, ap.due_date::text AS due_date, ap.created_at, ap.updated_at`,
      values
    );
    return result;
  },
